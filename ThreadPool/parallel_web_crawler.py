...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as group_link_threads:
    for i in range(urls.qsize()):
        group_link_threads.submit(group_urls_task, urls)

with concurrent.futures.ThreadPoolExecutor(max_workers=4) as crawler_link_threads:
...
print('End crawling:',time_execution)   

for url, links in result_dict.items():
    for link in links:
        logger.info("[%s] with links : [%s..." % (url, link))